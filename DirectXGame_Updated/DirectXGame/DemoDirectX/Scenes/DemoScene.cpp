...
void DemoScene::CheckCollision(float dt) const
{
	...
	for (auto& i : listEntitiesCollision)
	{
		...
		{
			Entity::SideCollisions entity_with_player;
			float collision_time = GameCollision::SweptAABB(gp->getBound(), i->getBound(), distance, entity_with_player);

			if (collision_time < 1.0f)
			{
				gp->UpdateColision(collision_time, entity_with_player, dt);
				gp->OnCollision(entity_with_player);
